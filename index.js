...
function Play(){  
  ...
  for(var i = 0; i < charEleArr.length; i++){
    RegisterChar(charEleArr[i],gridMap[i]);
...
function Create(){  
  ...
  if (mode === 0){   
    mode = 1;
    document.getElementById("crossword").innerHTML = GridToHtml(" ");    
...
function Generate(){  
  ...
  for(var i = 0, isSuccess=false; i < 19 && !isSuccess; i++){
    CleanVars();
    isSuccess = PopulateGrid();
  }

  document.getElementById("crossword").innerHTML = 
    (isSuccess) ? GridToHtml(" ") : "Failed to find crossword." ;
...
function AddWordToGrid(){
              ...
              for(var m = -1, lenM = curWord.char.length + 1; m < lenM; m++){
                ...
                if (m !== j){
                  if (curCross.dir === 0){
                    ...
                    crossVal.push(grid[xIndex][curCross.y].value);
                    crossVal.push(grid[xIndex][curCross.y + 1].value);
                    crossVal.push(grid[xIndex][curCross.y - 1].value);
                  }
                  else{
                    var yIndex = curCross.y + m;
                    
                    if (yIndex < 0 || yIndex > grid[curCross.x].length){
                      isMatch = false;
                      break;
                    }
                    
                    crossVal.push(grid[curCross.x][yIndex].value);
                    crossVal.push(grid[curCross.x + 1][yIndex].value);
                    crossVal.push(grid[curCross.x - 1][yIndex].value);
                  }
  ...
  for(i = 0, len = wordsActive[pushIndex].char.length; i < len; i++){
    ...
    cIndex = grid[xInd][yInd].char.length;
    
    grid[xInd][yInd].char.push(cObj);
    grid[xInd][yInd].value = wordsActive[pushIndex].char[i];
    
    Bounds.Update(xInd,yInd);
    
    if (prevObj !== null){
      prevObj.next = grid[xInd][yInd].char[cIndex];
    }
    
    prevObj = grid[xInd][yInd].char[cIndex];   
  }
...
function GridCharClick(gridChar){
  if (mode === 1){
    return;
  }
  
  if (gridChar.char.length > 1){
    if (focusIndex >= gridChar.char.length-1){
      focusIndex = 0;
    }
    else{
      focusIndex++;
...
function GridCharFocus(gridChar){
  if (mode === 1){
    return;
  }
  
  if (!(gridChar.char[focusIndex] && gridChar.char[focusIndex].prev === focusChar)){
    focusIndex = Math.max(0,gridChar.char.indexOf(focusChar));
  }  
  ...
  this.onkeydown = function(e){
    ...
    if (key === 8){
      if (gridChar.char[focusIndex].prev != null){
        focusChar = gridChar.char[focusIndex].prev;
        
        var isEnd = (gridChar.char[focusIndex].next === null)?true:false;
        
        document.getElementsByClassName('char')
        [gridChar.char[focusIndex].prev.index].focus();
        
        if (isEnd){
          document.getElementsByClassName('char')
          [gridChar.char[focusIndex].index].value = "";
          document.getElementsByClassName('char')
          [gridChar.char[focusIndex].next.index].value = "";
        }
      }
    }
  }
  ...
  this.onkeypress = function(){
    ...
    if (gridChar.char[focusIndex].next !== null){
      focusChar = gridChar.char[focusIndex].next;
      document.getElementsByClassName('char')
      [gridChar.char[focusIndex].next.index].focus();
    }
  }
}
...
function RegisterChar(ele, gridChar) {
  ele.onclick = CreateCallback("click", gridChar);
  ele.onfocus = CreateCallback("focus", gridChar);
}

function CreateCallback(type, gridChar) {
  switch(type){
    case "click":
      return function() {
        GridCharClick(gridChar);
      };
    case "focus":
      return function() {
        GridCharFocus(gridChar);
      };
  }
}
